...
public class AudioDeviceMonitor {
        ...
        synchronized (deviceChangeListeners) {
            for (DeviceChangeListener listener : deviceChangeListeners) {
                try {
                    ...
                    SoundSwitcher.logger.error("通知设备变化监听器失败", e);
                }
            }
        }
    }
    ...
    public void start() {
        synchronized (lock) {
            if (isRunning) {
                SoundSwitcher.logger.warn("音频设备监控器已在运行");
                return;
            }

            isRunning = true;
            SoundSwitcher.logger.info("启动音频设备监控器，检查间隔: " + CHECK_INTERVAL + "ms");
    ...
    public void stop() {
        synchronized (lock) {
            ...
            isRunning = false;
            monitorTimer.cancel();
            SoundSwitcher.logger.info("音频设备监控器已停止");
        }
    }
    ...
    private void performCheckInternal(boolean forceUpdate) {
        try {
            ...
            SoundSwitcher.logger.error("检测音频设备时发生错误", e);
        }
    }
    ...
    private Set<AudioDeviceInfo> getPlaybackDevices() {
        ...
        try {
            ...
            for (Mixer.Info mixerInfo : mixerInfos) {
                try {
                    ...
                    if (mixer.isLineSupported(lineInfo)) {
                        ...
                        SoundSwitcher.logger.debug("检测到播放设备: " + mixerName + 
                            (isDefault ? " [默认]" : ""));
                    }
                } catch (Exception e) {
                    // 忽略无法访问的混音器
            ...
            SoundSwitcher.logger.error("获取播放设备失败", e);
        }
    ...
    private Set<AudioDeviceInfo> getRecordingDevices() {
        ...
        try {
            ...
            SoundSwitcher.logger.error("获取录音设备失败", e);
        }
    ...
    private void logDeviceChanges(Set<AudioDeviceInfo> playbackDevices, 
        ...
        try {
            ...
            writer.println("========================================");
            writer.println("音频设备检测 - " + timestamp);
            writer.println("========================================");
            ...
            SoundSwitcher.logger.info("音频设备变化已记录到: " + logFile.getAbsolutePath());
        } catch (IOException e) {
            SoundSwitcher.logger.error("写入音频设备日志失败", e);
        }
    }
    ...
    private void checkLogFileSize() {
        if (logFile.exists() && logFile.length() > MAX_LOG_SIZE) {
            ...
            SoundSwitcher.logger.info("音频设备日志文件已备份");
        }
    }
    ...
    public String switchToNextPlaybackDevice() {
        synchronized (lock) {
            if (playbackDeviceList.isEmpty()) {
                SoundSwitcher.logger.warn("没有可用的播放设备");
                return null;
            }
            ...
            SoundSwitcher.logger.info("切换到音频输出设备: " + newDevice.getName());
        ...
        synchronized (lock) {
            for (int i = 0; i < playbackDeviceList.size(); i++) {
                if (playbackDeviceList.get(i).getName().equals(deviceName)) {
                    ...
                    SoundSwitcher.logger.info("切换到音频输出设备: " + deviceName);
            ...
            SoundSwitcher.logger.warn("找不到音频设备: " + deviceName);
            return false;
        }
    }
    ...
    private void switchMixer(AudioDeviceInfo device) {
        try {
            ...
            if (targetMixerInfo == null) {
                SoundSwitcher.logger.error("找不到混音器: " + device.getName());
                return;
            }
            ...
            SoundSwitcher.logger.error("切换混音器失败", e);
        }
    }
    ...
    private void switchMixerViaPaulscode(Mixer mixer) {
        try {
            ...
            SoundSwitcher.logger.info("成功通过 paulscode 切换混音器到: " + mixer.getMixerInfo().getName());
            
        } catch (ClassNotFoundException e) {
            // paulscode 不可用，尝试其他方式
            SoundSwitcher.logger.warn("paulscode LibraryJavaSound 不可用，尝试其他方式");
            switchMixerViaJavaSound(mixer);
        } catch (Exception e) {
            SoundSwitcher.logger.error("通过 paulscode 切换混音器失败", e);
        }
    }
    ...
    private void switchMixerViaJavaSound(Mixer mixer) {
        // JavaSound 本身不支持直接切换系统默认设备
        // 这里我们只是记录用户的设备偏好
        SoundSwitcher.logger.info("已记录音频设备偏好: " + mixer.getMixerInfo().getName());
    }
}
